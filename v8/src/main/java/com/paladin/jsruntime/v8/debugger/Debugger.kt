...
import com.paladin.jsruntime.v8.debugger.utils.LogUtils
...
internal class Debugger(
    ...
    private var dtoMapper: ObjectMapper = ObjectMapper()
    ...
    fun initialize(handlerThread: HandlerThread , v8Messenger: V8Messenger) {
    ...
    fun setBreakpointsActive(peer: JsonRpcPeer, params: JSONObject) {
        runStethoAndV8Safely {
            handler?.post { v8Messenger?.sendMessage(Protocol.Debugger.SetBreakpointsActive, params) }
        }
    }


    /**
     * Pass through to v8 methods
     */
    @Suppress("unused", "UNUSED_PARAMETER")
    @ChromeDevtoolsMethod
    ...
    fun setScriptSource(peer: JsonRpcPeer, params: JSONObject) {
        runStethoAndV8Safely {
            handler?.post { v8Messenger?.setScriptSource(Protocol.Debugger.SetScriptSource, params) }
        }
    }
